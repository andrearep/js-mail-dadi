...
if (playerDado > tavoloDado) {
    var msg = document.getElementById("msg").innerHTML = "complimenti hai vinto, ti sono stati accreditati i danari";
    saldo += (puntata * 2);
    //var saldoAttuale = document.getElementById("saldo").innerHTML = saldo + " €"

} else if (playerDado < tavoloDado) {
    var msg = document.getElementById("msg").innerHTML = "dispiace molto, puoi provare a puntare il doppio per rifarti";

} else {
    var msg = document.getElementById("msg").innerHTML = "oh no un pareggio, non c'è niente di peggio! comunque ci prendiamo 1 € per la corrente e i server";
    //saldo += puntata;
    // saldo--;
    // var saldoAttuale = document.getElementById("saldo").innerHTML = saldo + " €"
}
